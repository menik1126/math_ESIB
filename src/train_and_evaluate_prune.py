...
def train_one_batch_vae(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
    ...
    if config.is_mask:
       encoder_outputs, problem_output, mean, logvar  = encoder(input_var, input_length, mask = seq_mask)
    else:
       encoder_outputs, problem_output, mean, logvar= encoder(input_var, input_length)
    ...
    return all_node_outputs, target, mean, logvar
...
def train_one_batch_mid(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                       encoder, predict, generate, merge):
     # sequence mask for attention
    seq_mask = []
    max_len = max(input_length)
    ...
    return all_node_outputs, target, problem_output

def train_prune_SWS_mid(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder, predict, generate, merge,
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
    ...
    with torch.no_grad():
        ...
        # branch with pruned network
        all_node_outputs2, target_noGrad, problem_output_noGrad = train_one_batch_mid(input_batch[pos], input_length[pos], target_batch[pos], target_length[pos], nums_stack_batch[pos], num_size_batch[pos], num_pos[pos], generate_nums, output_lang,
                                        encoder1, predict1, generate1, merge1) 
        all_node_outputs2_noGrad = torch.stack(all_node_outputs2, dim=1).detach()
        problem_output_noGrad = problem_output_noGrad.detach()
    ...
    # 这个是总的loss
    #print("parameters_to_prune2:{}".format(parameters_to_prune[1]))
    all_node_outputs1, target, problem_output = train_one_batch_mid(input_batch[1-pos], input_length[1-pos], target_batch[1-pos], target_length[1-pos], nums_stack_batch[1-pos], num_size_batch[1-pos], num_pos[1-pos], generate_nums, output_lang,
                                       encoder, predict, generate, merge)
    all_node_outputs1 = torch.stack(all_node_outputs1, dim=1)  # B x S x N
    ...
    if config.is_mid_loss:
        loss, kl_loss, vae_loss = masked_cross_entropy(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature, problem_output=problem_output, problem_output_noGrad=problem_output_noGrad)
    else:
        loss, kl_loss = masked_cross_entropy(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature)
...
def train_prune_SWS_divide_unsym_0(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder, predict, generate, merge,
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
    ...
    # 这个是总的loss
    #print("parameters_to_prune2:{}".format(parameters_to_prune[1]))
    all_node_outputs1, target, mean, logvar = train_one_batch_vae(input_batch[1-pos], input_length[1-pos], target_batch[1-pos], target_length[1-pos], nums_stack_batch[1-pos], num_size_batch[1-pos], num_pos[1-pos], generate_nums, output_lang,
                                       encoder, predict, generate, merge)
    all_node_outputs1 = torch.stack(all_node_outputs1, dim=1)  # B x S x N
    ...
    loss, kl_loss, vae_loss = masked_cross_entropy_unsym(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature, mean=mean, logvar=logvar)
...
def train_prune_SWS_divide_unsym_1(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder, predict, generate, merge,
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
    ...
    with torch.no_grad():
        ...
        # branch with pruned network
        all_node_outputs2, target_noGrad, _, _ = train_one_batch_vae(input_batch[pos], input_length[pos], target_batch[pos], target_length[pos], nums_stack_batch[pos], num_size_batch[pos], num_pos[pos], generate_nums, output_lang,
                                        encoder1, predict1, generate1, merge1) 
        all_node_outputs2_noGrad = torch.stack(all_node_outputs2, dim=1).detach()
    ...
    loss, kl_loss = masked_cross_entropy_unsym(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature)
...
def train_tree_SWS_divide_unsym(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    # if alternate_flag == True:
    loss_prue, kl_loss2 = train_prune_SWS_divide_unsym_1(input_batch, input_length, target_batch, target_length, nums_stack_batch_copy, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder, predict, generate, merge,
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
                                encoder1, predict1, generate1, merge1,
                                
                                pos = 1, start = False)
    loss_no_prue, kl_loss1, vae_loss = train_prune_SWS_divide_unsym_0(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder1, predict1, generate1, merge1,
                                encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
                                encoder, predict, generate, merge,
                                pos = 0, start = True)
    ...
    return loss_no_prue.item(), kl_loss1.item() , loss_prue.item(), kl_loss2.item(), vae_loss 

def train_tree_SWS_divide_mid(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, 
               encoder1, predict1, generate1, merge1,
               encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, 
               encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
    ...
    # if alternate_flag == True:
    loss_no_prue, kl_loss1, vae_loss1 = train_prune_SWS_mid(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder, predict, generate, merge,
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
                                encoder1, predict1, generate1, merge1,
                                pos = 0, prune_start = False)

    loss_prue, kl_loss2, vae_loss2 = train_prune_SWS_mid(input_batch, input_length, target_batch, target_length, nums_stack_batch_copy, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder1, predict1, generate1, merge1,
                                encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
                                encoder, predict, generate, merge,
                                pos = 1, prune_start = False)
    ...
    return loss_no_prue.item(), kl_loss1.item() , loss_prue.item(), kl_loss2.item() , vae_loss1.item(), vae_loss2.item()
...
def evaluate_tree_vae(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
                  beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    # Run words through encoder
    if config.is_mask:
        encoder_outputs, problem_output, _,_ = encoder(input_var, [input_length], mask = seq_mask)
    else:
        encoder_outputs, problem_output, _,_  = encoder(input_var, [input_length])
