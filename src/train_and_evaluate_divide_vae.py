...
def train_one_batch_vae(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
    ...
    return all_node_outputs, target, mean, logvar, problem_output_sample
...
def train_tree_comp_noVAE(input_batch, input_length, target_batch, target_length, 
    ...
    loss = masked_cross_entropy_comp_noVAE(all_node_outputs, target, target_length)
    # loss = loss_0 + loss_1
    loss.backward()
    ...
    return loss.item(), 0  # , loss_0.item(), loss_1.item()
...
def train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
    ...
    with torch.no_grad():
        ...
        # branch with pruned network
        all_node_outputs2, target_noGrad, mean_noGrad, logvar_noGrad, problem_output_sample_noGrad = train_one_batch_vae(input_batch[pos], input_length[pos], target_batch[pos], target_length[pos], nums_stack_batch[pos], num_size_batch[pos], num_pos[pos], generate_nums, output_lang,
                                        encoder1, predict1, generate1, merge1) 
        all_node_outputs2_noGrad = torch.stack(all_node_outputs2, dim=1).detach()
    ...
    all_node_outputs1, target, mean, logvar, problem_output_sample = train_one_batch_vae(input_batch[1-pos], input_length[1-pos], target_batch[1-pos], target_length[1-pos], nums_stack_batch[1-pos], num_size_batch[1-pos], num_pos[1-pos], generate_nums, output_lang,
                                       encoder, predict, generate, merge)
    all_node_outputs1 = torch.stack(all_node_outputs1, dim=1)  # B x S x N
    ...
    loss, kl_loss, vae_loss = masked_cross_entropy(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature, mean=mean, logvar=logvar)
...
def evaluate_tree_comp(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
                  beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
