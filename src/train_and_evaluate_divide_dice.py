from src.masked_cross_entropy_divide_vae import *
...
def train_one_batch_vae(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
    ...
    if config.is_mask:
       encoder_outputs, problem_output, problem_output_sample, mean, logvar = encoder(input_var, input_length, mask = seq_mask)
    else:
       encoder_outputs, problem_output, problem_output_sample, mean, logvar = encoder(input_var, input_length)

    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output_sample.split(1, dim=0)]

    max_target_length = max(target_length)
    
    # 用来逼近最后的argmax参数
    all_node_outputs = []
    all_node_outputs_embeddings = []
    ...
    for t in range(max_target_length):
        ...
        current_embeddings \
        = predict( node_stacks, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, seq_mask, num_mask)
        ...
        all_node_outputs_embeddings.append(current_embeddings)
    ...
    # mean_all_node_outputs_embeddings = torch.mean(all_node_outputs_embeddings, dim = 1).squeeze(1)
    # mean = self.latent_linear_mean(mean_all_node_outputs_embeddings)
    # logvar = self.latent_linear_logvar(mean_all_node_outputs_embeddings)

    return all_node_outputs, target, mean, logvar, problem_output_sample , problem_output, \
           all_node_outputs_embeddings
...
def train_prune_SWS(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
    ...
    if parameters_to_prune is not None:
       ...
       with torch.no_grad():
            ...
            # branch with pruned network
            all_node_outputs2, target_noGrad = train_one_batch_dice(input_batch[pos], input_length[pos], target_batch[pos], target_length[pos], nums_stack_batch[pos], num_size_batch[pos], num_pos[pos], generate_nums, output_lang,
                                            encoder, predict, generate, merge) 
            all_node_outputs2_noGrad = torch.stack(all_node_outputs2, dim=1).detach()
    ...
    # 这个是总的loss
    #print("parameters_to_prune2:{}".format(parameters_to_prune[1]))
    all_node_outputs1, target = train_one_batch_dice(input_batch[1-pos], input_length[1-pos], target_batch[1-pos], target_length[1-pos], nums_stack_batch[1-pos], num_size_batch[1-pos], num_pos[1-pos], generate_nums, output_lang,
                                       encoder, predict, generate, merge)
    all_node_outputs1 = torch.stack(all_node_outputs1, dim=1)  # B x S x N
    ...
    loss, kl_loss = masked_cross_entropy_dice(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature)
...
def train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
    ...
    with torch.no_grad():
        ...
        # branch with pruned network
        all_node_outputs2, target_noGrad, mean_noGrad, logvar_noGrad, problem_output_sample_noGrad, _, all_node_outputs_embeddings_noGrad = train_one_batch_vae(input_batch[pos], input_length[pos], target_batch[pos], target_length[pos], nums_stack_batch[pos], num_size_batch[pos], num_pos[pos], generate_nums, output_lang,
                                        encoder1, predict1, generate1, merge1) 
        all_node_outputs2_noGrad = torch.stack(all_node_outputs2, dim=1).detach()
        problem_output_sample_noGrad = problem_output_sample_noGrad.detach()
        all_node_outputs_embeddings_noGrad = torch.stack(all_node_outputs_embeddings_noGrad, dim=1).detach()
    ...
    # 这个是总的loss
    #print("parameters_to_prune2:{}".format(parameters_to_prune[1]))
    all_node_outputs1, target, mean, logvar, problem_output_sample, problem_output, all_node_outputs_embeddings = train_one_batch_vae(input_batch[1-pos], input_length[1-pos], target_batch[1-pos], target_length[1-pos], nums_stack_batch[1-pos], num_size_batch[1-pos], num_pos[1-pos], generate_nums, output_lang,
                                       encoder, predict, generate, merge)
    all_node_outputs1 = torch.stack(all_node_outputs1, dim=1)  # B x S x N
    all_node_outputs_embeddings = torch.stack(all_node_outputs_embeddings , dim=1)
    if config.is_CEB_detached:
       all_node_outputs_embeddings = all_node_outputs_embeddings.detach()
       
    if config.is_Cross:
       all_node_outputs_embeddings = all_node_outputs_embeddings_noGrad
    ...
    if config.is_v:
        loss, kl_loss, vae_loss, CEB_loss = masked_cross_entropy_dice(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature, mean=mean, logvar=logvar, is_loss_no_mask = is_no_mask, problem_output_sample=problem_output,
                             all_node_outputs_embeddings  = all_node_outputs_embeddings )
    else:
        loss, kl_loss, vae_loss, CEB_loss = masked_cross_entropy_dice(all_node_outputs1, target, target_length[1-pos], logits_noGrad= all_node_outputs2_noGrad, target_noGrad = target_noGrad, length_noGrad = target_length[pos], temperature = config.temperature, mean=mean, logvar=logvar, is_loss_no_mask = is_no_mask, problem_output_sample=problem_output_sample,
                             all_node_outputs_embeddings  = all_node_outputs_embeddings )
    ...
    return loss, kl_loss, vae_loss, CEB_loss
...
def train_tree_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    loss_no_prue, kl_loss1, vae_kl1, CEB_loss1 = train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder, predict, generate, merge,
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
                                encoder1, predict1, generate1, merge1,
                                pos = 0, prune_start = False)

    loss_prue, kl_loss2, vae_kl2, CEB_loss2 = train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch_copy, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder1, predict1, generate1, merge1,
                                encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
                                encoder, predict, generate, merge,
                                pos = 1, prune_start = False)
    ...
    return loss_no_prue.item(), kl_loss1.item() , loss_prue.item(), kl_loss2.item(), vae_kl1.item(), vae_kl2.item(), CEB_loss1.item(), CEB_loss2.item()



def train_tree_SWS_divide_unsym(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, 
               encoder1, predict1, generate1, merge1,
               encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, 
               encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
    ...
    #if alternate_flag == True:
    loss_no_prue, kl_loss1, vae_kl1 = train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder, predict, generate, merge,
                                encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,
                                encoder1, predict1, generate1, merge1,
                                pos = 0, prune_start = False, is_no_mask = False)
    # unsym0 是True False
    # unsym1 是False True
    loss_prue, kl_loss2, vae_kl2 = train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch_copy, num_size_batch, num_pos, generate_nums, output_lang,
                                encoder1, predict1, generate1, merge1,
                                encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
                                encoder, predict, generate, merge,
                                pos = 1, prune_start = False, is_no_mask = True)
    # else:
    #     loss_prue, kl_loss2 = train_prune_SWS_divide(input_batch, input_length, target_batch, target_length, nums_stack_batch_copy, num_size_batch, num_pos, generate_nums, output_lang,
    #                                 encoder1, predict1, generate1, merge1,
    #                                 encoder_optimizer1, predict_optimizer1, generate_optimizer1, merge_optimizer1,
    ...
    return loss_no_prue.item(), kl_loss1.item() , loss_prue.item(), kl_loss2.item(), vae_kl1.item(), vae_kl2.item()
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
    ...
    # Run words through encoder
    if config.is_mask:
        encoder_outputs, problem_output, problem_output_sample, _, _ = encoder(input_var, [input_length], mask = seq_mask)
    else:
        encoder_outputs, problem_output, problem_output_sample, _, _ = encoder(input_var, [input_length])
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings,_ = predict(
